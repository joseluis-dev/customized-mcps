...
7. Validate: `curl -sS http://127.0.0.1:3001/healthz` returns `200 ok`.
...
4. **Allow `/healthz` through unauthenticated**. The probe returns `200 ok` when the app is ready and `503 unhealthy` once SIGTERM is received. The probe MUST be reachable from the orchestrator (Kubernetes liveness/readiness, load balancer health check, etc.).
...
| Agent auth | `MCP_AGENT_HMAC_SECRET`, `MCP_AGENTS_JSON` or `MCP_AGENTS_INLINE` | `.env` (HMAC secret) + JSON file or inline string |
...
- `GET /healthz` returns `200` with body `ok` when the app is ready to serve, and `503` with body `unhealthy` once SIGTERM is received or the factory fails to initialise. The endpoint is unauthenticated; the reverse proxy MUST allow it through.
...
- [ ] `curl -sS http://127.0.0.1:3001/healthz` returns `200 ok`.
