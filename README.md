...
```json
{
  "mcpServers": {
    "readonly-sql": {
      "command": "node",
      "args": ["D:/path/to/this/project/dist/index.js"],
      "env": {
        "DOTENV_CONFIG_PATH": "D:/path/to/this/project/.env"
      }
    }
  }
}
...
The server reads `.env` from the current working directory by default. `DOTENV_CONFIG_PATH` is honored by `dotenv` to point at a specific file.
